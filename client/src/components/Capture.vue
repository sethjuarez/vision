<template>
  <div>
    <div id="instructions">
      ...
      <p>
        Once you uploaded a balanced set of pictures for each category head over
        to <a href="customvision.ai">customvision</a> to train and download the
        model
      </p>
    </div>
    ...
    <div id="images">
      ...
      <div id="output">
        <div id="flavor" v-if="modelmeta != null">
          Type: {{ modelmeta.Flavor }}
        </div>
        <div id="exported" v-if="modelmeta != null">
          Exported: {{ modelmeta.ExportedDate }}
        </div>
        <div id="current">{{ guess }}</div>
        <div id="plist">
          <ul>
            <li :key="idx" v-for="(pitem, idx) in probabilities">
              {{ pitem.label }}: {{ pitem.probability.toFixed(2) }}%
            </li>
          </ul>
        </div>
      </div>
    ...
    <div id="listOPics" v-if="list.length > 0">
      ...
      <div>
        (Also maybe clean out any images that might be ambiguous if possible)
      </div>
...
export default {
  ...
  methods: {
    ...
    submitImages: async function() {
      ...
      // api endpoint
      try {
        let url = `${process.env.VUE_APP_API}/save`;
        let max_submit = 4;
